(function () {
    ...
    function initializeSocket(signalingURL) {
        ...
        socket.on('message', function (data) {
            ...
            switch (data.type) {
                ...
                case 'mute':
                    ...
                    $('#toggleMic').html('<i class="fa fa-microphone-slash"></i>').removeClass('green');
                ...
                case 'volume_up':
                    ...
                    $('#toggleMic').html('<i class="fa fa-microphone"></i>').addClass('green');
                    micMuted = false;
                    break;
                case 'screenSharing':
                    if (data.start == 'started') {
                        user_sharing_screen = '#container-' + data.socket_id;
                        $('#video-' + data.socket_id).attr("style", "-webkit-transform: scaleX(1) !important;transform:scaleX(1) !important");
        ...
        if (userInfo && !userInfo.username) {
            userInfo.username = username.value = localStorage.getItem('username') || settings.defaultUsername;
        }
    ...
    function setToWatching() {
        ...
        if (localStream) {
            ...
            $('#toggleVideo').html('<i class="fa fa-video-slash"></i>').removeClass('green');
            $('#toggleMic').html('<i class="fa fa-microphone-slash"></i>').removeClass('green');
        ...
        if (data.result) {
            ...
            try {
                ...
                if (muteAudio.checked) {
                    ...
                    $(this).html('<i class="fa fa-microphone-slash"></i>').removeClass('green');
                ...
                showError('Unable to get devices, please check permissions and try again. mistake: ' + e.name);
                $('#joinMeeting').attr('disabled', false);
                $("#watchMeeting").attr('disabled', false);
            }
    ...
    function startRecording() {
        if (user_sharing_screen === -1 && user_speaking_show !== '.localVideoContainer') {
    ...
    function hideOptions() {
        $('.meeting-options, .local-user-name, .remote-user-name, .meeting-info, .kick, .full-screen').hide();
    }

    //show meeting ID and options
    function showOptions() {
        $('.meeting-options, .local-user-name, .remote-user-name, .meeting-info, .kick, .full-screen').show();
    ...
    function refreshMemnbers() {
        ...
        Object.keys(usernames).forEach((socketId, index, array) => {
            ...
            if (isModerator) {
                ...
                str +=
                    '<div class="member">\n' +
                    '<span>' + usernames[socketId].username + '</span>\n';

                str += '<a href="#" data-id="' + socketId + '" class="member-btn kick_off"><i class="fa fa-ban"></i></a>';

                if (!usernames[socketId].micMuted) {
                    str += '<a href="#" data-id="' + socketId + '" class="member-btn volume_off"><i class="fa fa-volume-up"></i></a>';
                } else {
                    str += '<a href="#" data-id="' + socketId + '" class="member-btn volume_up"><i class="fa fa-volume-off"></i></a>';
                }

                if (usernames[socketId].watch) {
                    str += '<a href="#" data-id="' + socketId + '" class="member-btn plus"><i class="fa fa-plus"></i></a>';
                } else {
                    str += '<a href="#" data-id="' + socketId + '" class="member-btn minus"><i class="fa fa-minus"></i></a>';
                }
                str += '</div>';
                ...
                $('.members-body').append('' +
                    '<div class="member">\n' +
                    '<span>' + usernames[socketId].username + '</span></div>');
            }
        });
    ...
    $(document).on('click', '.plus', function () {
        if (confirm('Are you sure you want to add this user to the meeting?')) {
            ...
            refreshMemnbers();
            setUpRemoteVideo(events[$(this).data('id')], $(this).data('id'), null, false);
            sendMessage({
                type: 'removeWatch',
                watch: false,
                socket_id: $(this).data('id'),
    ...
    $(document).on('click', '.minus', function () {
        if (confirm('Are you sure you want to pause this user from the meeting?')) {
            ...
            refreshMemnbers();
            setUpRemoteVideo(events[$(this).data('id')], $(this).data('id'), null, true);
            sendMessage({
                type: 'removeWatch',
                watch: true,
                socket_id: $(this).data('id'),
    ...
    function stopMic(attr) {
        ...
        if (micMuted) {
            ...
            localStream.getAudioTracks().forEach((track) => (track.enabled = false));
            $(attr).html('<i class="fa fa-microphone-slash"></i>').removeClass('green');
    ...
    async function startScreenSharing() {
        ...
        try {
            ...
            $('#videos div').removeClass('hide');
            layout();
            showError('Unable to share screen, please check permissions and try again.');
        }
        ...
        if (screenStream) {
            ...
            $('#localVideo').attr("style", "-webkit-transform: scaleX(1) !important;transform:scaleX(1) !important");
            sendMessage({
                type: 'screenSharing',
                socket_id: socket.id,
                start: 'started'
