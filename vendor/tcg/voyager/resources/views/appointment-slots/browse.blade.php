...
@else
    @section('page_title', "Manage Appointement payment list")
@endif
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            ...
            @else
                {{ "Manage Appointement payment list" }}
            @endif
        </h1>
...
@section('content')
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        @if (auth('web')->user()->role_id == getRoleID('provider') && $row->getTranslatedAttribute('display_name')=='Provider')
                                        @else
                                            <th>
    ...
    $row->type == 'relationship' && $row->field=='appointment_slot_belongsto_provider_relationship')
                                        ...
                                        <td style="width: 200px;">
                                            @php
                                                $times = \App\Models\AppointmentSlotTimes::where('slot_id',$data->{'id'})->get();
                                                @endphp
                                            @foreach($times as $t)
                                                <div>{{\Carbon\Carbon::parse($t->from_time)->format('h:i A').' - ' .\Carbon\Carbon::parse($t->to_time)->format('h:i A')}}</div>
                                            @endforeach
                                        </td>
