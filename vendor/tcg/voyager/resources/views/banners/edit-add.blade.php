...
@section('javascript')
    <script>
        ...
        var resource_id = '{{($dataTypeContent->{'resource_id'})}}';

        function updateResourceId() {
            var resource_type = $('select[name="resource_type"]').val();
            $.getJSON("{{route('update-banner-resource-id')}}/" + resource_type, function (data) {
                // $('select[name="resource_id"]').select2({
                //     multiple: false,
                //     data: data
                // });
                $('select[name="resource_id"]').empty();
                var $element = $('select[name="resource_id"]').select2();
                for (var d = 0; d < data.length; d++) {
                    var item = data[d];

                    // Create the DOM option that is pre-selected by default

                    var option = new Option(item.text, item.id, false, (parseInt(resource_id) == item.id) ? true : false);
        ...
        $('select[name="resource_type"]').change(() => {
            updateResourceId();
        });
