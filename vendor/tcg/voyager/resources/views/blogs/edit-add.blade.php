...
@section('page_title', __('voyager::generic.'.($edit ? 'edit' : 'create')).' '.$dataType->getTranslatedAttribute('display_name_singular'))

@section('page_header')
    <h1 class="page-title">
        <i class="{{ $dataType->icon }}"></i>
        {{ __('voyager::generic.'.($edit ? 'edit' : 'create')).' '.$dataType->getTranslatedAttribute('display_name_singular') }}
    </h1>
...
@section('javascript')
    ...
    <script>
        ...
        function updateSubCategories() {
            ...
            $.getJSON("{{route('update-sub-categories')}}/" + category_id, function (data) {
                $('select[name="sub_category_id"]').empty();
                var $element = $('select[name="sub_category_id"]').select2();
                for (var d = 0; d < data.length; d++) {
                    var item = data[d];

                    // Create the DOM option that is pre-selected by default

                    var option = new Option(item.text, item.id, false, (parseInt(category_id) == item.id) ? true : false);
...
{{--        @endif--}}
    ...
    <script>
        ...
        var cropper;
        $("body").on("change", "input[name='banner']", function(e){
        ...
        $("#crop").click(function(){
            ...
            input_images=document.querySelector("input[name='banner']")
