...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('edit', app($dataType->model_name))
            ...
            @endif
        @endcan
        <input type="checkbox" @if ($showPastLivestreams) checked @endif id="show_past_livestreams" data-toggle="toggle"
               data-on="Previous" data-off="Current">
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-8">
                                <form method="get" id="goLiveForm" action="{{route('go-live')}}">
                                    <div class="row">
                                        <div class="col-md-6 form-group">
                                            <label class="control-label">
                                                Select Live Now Catagory
                                            </label>
                                            <select class="form-control" name="live_now_category_id"
                                                    required
                                                    id="live_now_category_id">
                                                @foreach(\App\Models\LiveNowCategory::where('platform_id',auth('web')->user()->platform_id)->get() as $cat)
                            ...
                            <div class="col-md-4 goLive" style="text-align: center;cursor: pointer;">
                                <img src="{{asset('widgets/live.svg')}}" width="30%">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    @if ($isServerSide)
                        ...
                        <div class="pull-right">
                            {{ $dataTypeContent->appends([
                                ...
                                'showPastLivestreams' => $showPastLivestreams,
                            ])->links() }}
                        </div>
                    @endif
                </div>
...
@section('javascript')
    ...
    <script>
        ...
        $(function () {
            ...
            $('#show_past_livestreams').change(function () {

                if ($(this).prop('checked')) {
                    $('#dataTable').before('<a id="redir" href="{{ (route('voyager.'.$dataType->slug.'.index', array_merge($params, ['showPastLivestreams' => 1]), true)) }}"></a>');
                } else {
                    $('#dataTable').before('<a id="redir" href="{{ (route('voyager.'.$dataType->slug.'.index', array_merge($params, ['showPastLivestreams' => 0]), true)) }}"></a>');
                }
