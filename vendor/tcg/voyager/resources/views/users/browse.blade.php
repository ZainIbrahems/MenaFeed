...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            @if(request()->has('u_t'))
                @if(request()->get('u_t')=='client')
                    <i class="{{ $dataType->icon }}"></i> Clients
                @elseif(request()->get('u_t')=='provider')
                    <i class="{{ $dataType->icon }}"></i> Providers
                @else
        ...
        @if(request()->has('u_t') && (request()->get('u_t')=='client'||request()->get('u_t')=='provider'))
        @else
            @can('add', app($dataType->model_name))
                <a href="{{ route('voyager.'.$dataType->slug.'.create') }}?u_t={{request()->get('u_t')}}"
                    ...
                    <div class="panel-body">
                        @if ($isServerSide)
                            <form method="get" class="form-search">
                                <input type="hidden" name="u_t" value="{{request()->get('u_t')}}">
                                <div id="search-input">
                                    <div class="col-2">
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            @foreach($actions as $action)
                                                @if (!method_exists($action, 'massAction'))
                                                    @include('voyager::bread.partials.actions_users', ['action' => $action])
                                                @endif
                                            @endforeach
                                        </td>
                                    </tr>
        ...
        $(function () {
            $('#show_soft_deletes').change(function () {
                if ($(this).prop('checked')) {
                    $('#dataTable').before('<a id="redir" href="{{ (route('voyager.'.$dataType->slug.'.index', array_merge($params, ['showSoftDeleted' => 1,'u_t'=>request()->get('u_t')]), true)) }}"></a>');
                } else {
                    $('#dataTable').before('<a id="redir" href="{{ (route('voyager.'.$dataType->slug.'.index', array_merge($params, ['showSoftDeleted' => 0,'u_t'=>request()->get('u_t')]), true)) }}"></a>');
                }
