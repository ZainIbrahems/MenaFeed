                                            ...
                                            <td>
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    @if($row->field=='user_id')
                                                        @if($data->{$row->field}=='provider')
                                                            <?php
                                                            $user = \App\Models\Provider::where('id', $data->{'user_id'})->first();
                                                            ?>
                                                        @else
                                                            <?php
                                                            $user = \App\Models\Client::where('id', $data->{'user_id'})->first();
                                                            ?>
                                                        @endif
                                                        {{$user?$user->full_name:''}}
                                                    @else
                                                        <div>
                                                            {{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' :
                                                        $data->{$row->field} }}</div>
                                                    @endif
