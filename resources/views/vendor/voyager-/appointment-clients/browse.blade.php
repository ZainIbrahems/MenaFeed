...
@else
    @section('page_title', "Providers Appointement list")
@endif
@section('page_header')
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        @if (auth('web')->user()->role_id == getRoleID('provider') && $row->getTranslatedAttribute('display_name')=='Professional')
                                        @else
                                            <th>
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            @endphp
                                            @if(
    auth('web')->user()->role_id == getRoleID('provider') &&
    $row->type == 'relationship' && $row->field=='appointment_client_belongsto_provider_relationship')
                                            @else
                                                <td>
                                                    ...
                                                    @elseif(($row->type == 'select_dropdown' || $row->type == 'radio_btn') && property_exists($row->details, 'options'))
                                                        @if($row->field=='payment_status')
                                                            <h6
                                                                ...
                                                                text-align: center;
    color: white;padding: 5px;border-radius: 11px;background-color: {{PaymentColor($data->{$row->field})}}">
                                                            {!! $row->details->options->{$data->{$row->field}} ?? '' !!}
                                                            </h6>
                                                        @elseif($row->field=='state')
                                                            <h6
                                                                style="background-color: #ff002d;
                                                                text-align: center;
    color: white;padding: 5px;border-radius: 11px;background-color: {{SateColor($data->{$row->field})}}">
                                                            {!! $row->details->options->{$data->{$row->field}} ?? '' !!}
                                                            </h6>
                                                        @else
                                                            {!! $row->details->options->{$data->{$row->field}} ?? '' !!}
