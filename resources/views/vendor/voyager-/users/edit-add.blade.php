@php
    ...
    $user = \App\Models\User::where('id',$dataTypeContent->getKey())->first();
@endphp
    ...
    <h1 class="page-title">
        @if(request()->has('u_t'))
            @if(request()->get('u_t')=='client')
                ...
                {{ __('voyager::generic.'.($edit ? 'edit' : 'add')).' Client'}}
            @elseif(request()->get('u_t')=='provider')

                <i class="{{ $dataType->icon }}"></i>
                {{ __('voyager::generic.'.($edit ? 'edit' : 'add')).' Provider' }}
            @else
                <i class="{{ $dataType->icon }}"></i>
...
{{--                                @php--}}
{{--                                    $show_in = property_exists($row->details,'show_in')? json_encode($row->details->show_in):'';--}}
{{--                                    $role_name = getRoleName(auth()->user()->role_id);--}}
{{--                                    $show = false;--}}
{{--                                    if(strlen($show_in)>0 &&strlen($role_name)>0&& str_contains($show_in,$role_name)){--}}
{{--                                        $show = true;--}}
{{--                                    }--}}
